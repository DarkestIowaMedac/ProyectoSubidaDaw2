...
export function FormLayout() {
    ...
    const [interpretaciones, setInterpretaciones] = useState([{ texto: "" }]);
    ...
    const borrarInterpretaciones = async (muestraId) => {
        try {
            ...
            const response = await fetch(
                `/ProyectoSubidaDaw2/public/borrarinterpretaciones/${muestraId}`,
                {
                    method: "DELETE",
                    headers: {
            ...
            if (response.ok) {
                ...
                console.error(
                    "Error al eliminar las interpretaciones:",
                    errorData
                );
                alert(
                    "Error al eliminar las interpretaciones: " +
            ...
            alert(
                "Error al realizar la solicitud para eliminar interpretaciones"
            );
        }
    };
    ...
    const borrarImagenes = async (muestraId) => {
        try {
            const csrfToken = document
                ...
                .getAttribute("content");
            const response = await fetch(
                `/ProyectoSubidaDaw2/public/borrarimagenes/${muestraId}`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const obtenerMuestraDeCookie = () => {
        ...
        const muestraCookie = cookies.find((cookie) =>
            cookie.startsWith("muestra=")
        );
        deleteAllCookies();
        if (muestraCookie) {
    ...
    const [muestraAnterior, setMuestraAnterior] = useState(
        obtenerMuestraDeCookie()
    );
    ...
    const obtenerId = async () => {
        try {
            ...
            alert(
                "Error al cargar los formatos. Por favor, intenta de nuevo más tarde."
            );
            return null;
        }
    };
            ...
            if (response.ok) {
                ...
                if (images.length > 0) {
                    ...
                    const imageResponse = await fetch(
                        `/ProyectoSubidaDaw2/public/crearimagenes`,
                        {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                    ...
                    console.log("API Response:", imageResponseData);
                    if (imageResponse.ok) {
                        ...
                        alert(
                            "Error al subir las imágenes: " + errorData.message
                        );
                    }
                }
                ...
                if (interpretaciones.length > 0) {
                    const interpretacionesToSend = interpretaciones.map(
                        (inter) => ({
                            muestra_id: muestraId,
                    ...
                    const interpretacionResponse = await fetch(
                        `/ProyectoSubidaDaw2/public/muestras/${muestraId}/interpretaciones`,
                        {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                            ...
                            body: JSON.stringify({
                                interpretaciones: interpretacionesToSend,
                            }), // Envía las interpretaciones
                        }
                    );
                    ...
                    if (interpretacionResponse.ok) {
                        ...
                        console.error(
                            "Error al subir las interpretaciones:",
                            errorData
                        );
                        alert(
                            "Error al subir las interpretaciones: " +
        ...
        <>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <AaImagen
                    images={images}
                    setImages={setImages}
                    muestraId={idComponent}
                />{" "}
                {/* Pasa la función para actualizar las URLs de las imágenes */}
                <br />
                <br />
                <AaInterpretacion
                    muestraId={idComponent}
                    interpretaciones={interpretaciones}
                    setInterpretaciones={setInterpretaciones}
                />
                <br />
                <br />
                <button
